...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add') {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO companies (user_id, name, address, description, logo) 
                VALUES (?, ?, ?, ?, ?)
            ");
            $stmt->execute([$userId, $name, $address, $description, $logo]);

            // Ghi log hoạt động
            $stmt = $pdo->prepare("INSERT INTO activities (user_id, description) VALUES (?, ?)");
            $stmt->execute([$_SESSION['user_id'], "Thêm công ty mới: $name"]);

            $pdo->commit();
            header('Location: companies_admin.php?success=1');
            exit;
        } catch (Exception $e) {
            $pdo->rollBack();
...
if (isset($_GET['delete'])) {
    ...
    try {
        ...
        // Ghi log hoạt động
        $stmt = $pdo->prepare("INSERT INTO activities (user_id, description) VALUES (?, ?)");
        $stmt->execute([$_SESSION['user_id'], "Xóa công ty ID: $companyId"]);

        $pdo->commit();
        header('Location: companies_admin.php?success=2');
        exit;
    } catch (Exception $e) {
        $pdo->rollBack();
        $error = "Lỗi: " . $e->getMessage();
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($company['logo'])): ?>
                                            <img src="uploads/companies/<?php echo htmlspecialchars($company['logo']); ?>" 
                                    ...
                                    <td>
                                        <a href="edit_company_admin.php?id=<?php echo $company['id']; ?>" class="btn btn-sm btn-primary">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="companies_admin.php?delete=<?php echo $company['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa công ty này?')">
